...
Sub Cust_Add()
    ...
    'Unhide customer additions pane
    With Sheets("Control Panel")
        ...
        If .Shapes("Item_Lookup_Pane").Visible = True Then
            Item_Lookup_Cancel
        End If
        .Shapes("Cust_Add_Pane").Visible = True
        .Shapes("Cust_Add_Listbox").Visible = True
        ...
        .Shapes("Cust_Add_Cancel").Visible = True
        .Shapes("Cust_Add_Select").Visible = True
    End With
    
    'Update list to be just non current customers
    With Sheets("DropDowns")
        Sheets("Control Panel").Cust_Add_Listbox.List = _
        .Range("I1:I" & .Range("I" & .Rows.Count).End(xlUp).Row + 1).value
    End With
...
Sub Cancel_Cust_Add()
    ...
    With ThisWorkbook.Sheets("Control Panel")
        .Shapes("Cust_Add_Listbox").Visible = False
        .Shapes("Cust_Add_Pane").Visible = False
        .Cust_Add_Listbox.Clear
        .Shapes("Cust_Add_Cancel").Visible = False
        .Shapes("Cust_Add_Select").Visible = False
        .Shapes("OpCo_List_Select").Visible = False
        .Shapes("DAB_Select").Visible = False
        .Shapes("Bulk_Select").Visible = False
    End With
...
Sub Select_Cust_Add()
    ...
    'Upload changes
    UL_Maintenance.Upload_Programs
    ...
    With Sheets("Programs")
        ...
        .Range("H1:H" & .Range("H" & .Rows.Count).End(xlUp).Row).EntireRow.Sort Key1:=.Range("H1"), Order1:=xlAscending, Header:=xlYes
        .Rows(1).AutoFilter
    End With
    ...
    'Pull in Customer Profile sheet additions
    rst.Open "SELECT * " _
        & "FROM UL_Customer_Profile " _
        & "WHERE CUSTOMER IN (" & strCust & ") " _
        & "ORDER BY CUSTOMER", cnn
    
    'Paste new records and resort
    With Sheets("Customer Profile")
        .Unprotect "Dac123am"
        .Rows(1).AutoFilter
        .Range("A" & .Range("A" & .Rows.Count).End(xlUp).Row + 1).CopyFromRecordset rst
        .Range("C1:C" & .Range("C" & .Rows.Count).End(xlUp).Row).EntireRow.Sort Key1:=.Range("C1"), Order1:=xlAscending, Header:=xlYes
        .Rows(1).AutoFilter
    End With
    ...
    'Remove added customer(s) from non assigned customer list and add to assigned list
    With Sheets("DropDowns")
        If InStr(strCust, ",") > 0 Then
            For i = 0 To UBound(Split(strCust, ","))
                .Range("H" & .Range("H" & .Rows.Count).End(xlUp).Row + 1).value = Replace(Split(strCust, ",")(i), "'", "")
                .Cells(.Columns(9).Find(Replace(Split(strCust, ",")(i), "'", "")).Row, 9).Delete
            Next
        Else
            .Range("H" & .Range("H" & .Rows.Count).End(xlUp).Row + 1).value = Replace(strCust, "'", "")
            .Cells(.Columns(9).Find(Replace(strCust, "'", "")).Row, 9).Delete
        End If
        
        'Create entry for drop down if none exist
        If .Cells(1, 9) = "" Then .Cells(1, 9).value = "(Blank)"
        
        'Sort columbs and update drop down list
        .Range("H1:H" & .Range("H" & .Rows.Count).End(xlUp).Row).Sort Key1:=.Range("H1"), Order1:=xlAscending, Header:=xlYes
        .Range("I1:I" & .Range("I" & .Rows.Count).End(xlUp).Row).Sort Key1:=.Range("I1"), Order1:=xlAscending, Header:=xlNo
        Sheets("Control Panel").Cust_Add_Listbox.List = _
        .Range("I1:I" & .Range("I" & .Rows.Count).End(xlUp).Row + 1).value
    End With
    ...
    'Ensure this is the ending page
    Sheets("Control Panel").Activate
